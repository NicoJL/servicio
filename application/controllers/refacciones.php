...
class Refacciones extends CI_Controller {
	...
	function consultaGeneral()
	{
		...
		if($id=="")
			$this->session->set_userdata('idsuc2',2); // valor por default (ingresar suc)
		else
	...
	public function vender()
	{
		...
		$this->pagination->initialize($config);
		$data['paginacion']=$this->pagination->create_links();
		$data['query']=$this->ModelRef->consultaGeneralRef(2,$offset,$config['per_page']);
		$data['query']->next_result();
		$data['title']="Refacciones";
		$data['cont']=$this->uri->segment($uri_segment);
	...
	function terminar()
	{
		...
		{
			$query=$this->ModelRef->venderRef($items['id'],$items['idServ'],2,$items['qty'],$items['price']);
